...
class TD3_car(object):
    def __init__(self, state_dim, action_dim, max_action):
        self.s_dim = state_dim       # 状态维度
        self.a_dim = action_dim      # 动作维度
        self.memory = np.zeros((MEMORY_CAPACITY, state_dim * 2 + action_dim + 1), dtype=np.float32)  # 记忆库
        ...
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=LR_ACTOR)  # actor 优化器
        self.scheduler_actor = torch.optim.lr_scheduler.StepLR(self.actor_optimizer, step_size=2000, gamma=0.5)#500

        self.critic = Critic(state_dim, action_dim)
        self.critic_target = Critic(state_dim, action_dim)
        self.critic_target.load_state_dict(self.critic.state_dict())
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=LR_CRITIC)
        self.scheduler_critic = torch.optim.lr_scheduler.StepLR(self.critic_optimizer, step_size=1500, gamma=0.5)#300
