...
class Environment(object):
    ...
    def set_time(self, light_time):
        self.light_time = light_time

    def change_time(self, index, time):
        self.light_time[index] += time
    ...
    def set_cars_acc(self, car_name, car_acc):
        ...
        if state[0] > 15 / MAX_DIS:
            ...
            if car_acc >= 0:  # 加速操作
                ...
                traci.vehicle.setSpeed(car_name, 3)
            return car_name
        else:
            # car_acc = -state[0]
            # traci.vehicle.setSpeedMode(car_name, 1110)  # 更改车辆速度模型，使得能够控制其速度
    ...
    def get_car_state(self, car):
        ...
        current_speed = traci.vehicle.getSpeed(car)  # 该车车速
        current_acc = traci.vehicle.getAcceleration(car)  # 该车加速度
        ...
        if traci.vehicle.getRoadID(car) in ['edge_0', 'edge_2']:
            ...
            cross_dis = -MAX_DIS  # 驶离路口不受红绿灯控制，距离设置为-1

        light_time = self.light_time[::3]
        index_phase = None
        for phase in range(len(light_time)):
            if light_time[phase] != 20 and light_time[phase] != 0:
                index_phase = phase
                break

        state_car_light = [0, 0]  # 第一个元素为当前相位是否是该车放行相位，第二个元素为该车放行相位剩余时间
        light_car = [0, 0, 0, 0]
        car_phase = -1
        ...
        if traci.vehicle.getLaneID(car) in ['edge_0_2', 'edge_2_2']:
            light_car = [0, 0, 0, 1]
            car_phase = 3
        if car_phase == index_phase:
            state_car_light[0] = 1

        light_car_state = [a * b for a, b in zip(light_car, light_time)]
        light_car_state = sum(light_car_state) / 20

        state_car_light[1] = light_car_state
        ...
        # state = [relative_dis/300, relative_speed/15, cross_dis/300, current_speed/15, current_acc]#.extend(light_car_state)
        state = [relative_dis / MAX_DIS, relative_speed / MAX_SPEED, cross_dis / MAX_DIS, current_speed / MAX_SPEED,
                 current_acc / 3]
        state.extend(state_car_light)
        # print(state)
    ...
    def get_light_state(self):
        ...
        for lane in lane_name:
            state_light.append(traci.lane.getLastStepVehicleNumber(lane)/10)
        # for lane in lane_name:
